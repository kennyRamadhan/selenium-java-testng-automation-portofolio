 ...
 *
 * <p>
 * Provides a {@link #given()} factory that returns a pre-configured RestAssured
 * {@link RequestSpecification} with:
 * </p>
 * <ul>
 ...
 * <li>An {@link AllureRestAssured} filter so every request/response is attached
 * to the Allure report as a step trace.</li>
 * </ul>
 *
 * <h2>automationexercise.com response quirk</h2>
 * <p>
 * This API <em>always</em> returns HTTP 200 — even for "errors" like
 ...
 * {@code responseCode} field. Tests must therefore inspect the body, not the
 * HTTP status, to determine success/failure. See
 * {@link com.kennyramadhan.qa.api.models.ApiResponse}.
 * </p>
 */
public abstract class BaseApiClient {
	...
	/**
	 * Build a fresh {@link RequestSpecification} for a single request. Each call
	 * returns a new spec so concurrent test threads do not share mutable builder
	 * state.
	 */
	protected RequestSpecification given() {
		return RestAssured.given()
				.spec(new RequestSpecBuilder().setBaseUri(ApiConfig.BASE_URL).setContentType(ContentType.URLENC)
						.setAccept(ContentType.JSON).build())
				.filter(new AllureRestAssured()).filter(new JsonResponseAllureFilter());
	}
}
