...
/**
 * <h1>AppiumServerManager</h1> This class manages the lifecycle of the Appium
 * Server (start & stop) and initializes the appropriate driver (iOS or Android)
 * based on the configuration provided in <code>config.properties</code>.
 *
 * <p>
 * <b>Usage:</b>
 * </p>
 ...
 *
 * <p>
 * Supports both iOS and Android automation without hardcoding node or appium
 * paths.
 * </p>
 *
 * @author Kenny
...
public class AppiumServerManager {
	 ...
	 * a default if neither is set.
	 *
	 * @param key
	 *            The property or environment variable name
	 * @param def
	...
	/**
	 * Resolves Appium's main.js path without hardcoding. Supports macOS, Linux, and
	...
	private static File resolveNodePath() {
		...
		String[] guesses = new String[]{"/usr/local/bin/node", "/opt/homebrew/bin/node",
	...
	/**
	 * Generates a random port number between 1000-9999 to avoid conflicts when
	 * starting Appium.
	 *
	 ...
	 *
	 * @throws IOException
	 *             if there is an error starting the service
	...
	/**
	 * Initializes AppiumDriver for either iOS or Android based on `platformName` in
	 * config.properties.
	 *
	 * <p>
		...
		while (driver == null && attempt < maxRetry) {
			...
			try {
				...
				if (platform.contains("ios")) {
					...
					log.info("[iOS] Driver initialized for: {}",
							ConfigLoader.getOrDefault("deviceName", "iPhone Simulator"));
				} else if (platform.contains("android")) {
					driver = new AndroidDriver(service.getUrl(), caps);
					log.info("[Android] Driver initialized for: {}",
							ConfigLoader.getOrDefault("deviceName", "Android Emulator"));
				} else {
