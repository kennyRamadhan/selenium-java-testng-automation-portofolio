...
public class WaitHelpers {
	...
	public void scrollIntoText(String text) {
		String platformName = DriverManager.getDriver().getCapabilities().getCapability("platformName").toString()
				.toLowerCase();
		...
		if (platformName.contains("android")) {
			...
			// iOS-specific scroll: loop until the element is found or max scroll attempts
			// reached
			boolean found = false;
	...
	/**
	 * Extract all prices from a list of WebElements, parse to Double, and log to
	 * the report.
	 *
	 * @param elements
	 *            list of WebElements that hold prices
	 * @param label
	 *            log label (e.g. "Before Sorting" or "After Sorting").
	...
	/**
	 * Tap element by its center coordinates (force tap). This works even if
	 * element.click() fails.
	 *
	 * <p>
	 * Uses {@code mobile: clickGesture} per Appium UiAutomator2 7.x; the older
	 * {@code mobile: tap} script was removed in favor of W3C-compliant gesture
	 * commands.
	 */
	public void tapByCoordinates(WebElement element, int offset) {
		...
		DriverManager.getDriver().executeScript("mobile: clickGesture", tapParams);
	}
